...
@section('filter-section')
    <x-filters.filter-box>
        ...
        <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
            ...
            <div class="select-status">
                <select class="form-control select-picker" id="filter-status">
                    ...
                    <option value="Not Yet Started">@lang('app.notYetStarted')</option>
        ...
        <!-- PRIORITY START -->
        <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
            <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('app.priority')</p>
            <div class="select-status">
                <select class="form-control select-picker" id="filter-priority">
                    <option value="all">@lang('app.all')</option>
    ...
    <div class="content-wrapper">
        ...
        <div class="d-flex justify-content-between action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center">
                <x-forms.link-primary :link="route('tasksassignmentmodule.create')" class="mr-3 float-left openRightModal"
                    icon="plus">
                    @lang('modules.tasks_assignment.addTaskAssignment')
...
@push('scripts')
    ...
    <script>
        $('#tasks-assignment-table').on('preXhr.dt', function (e, settings, data) {
        ...
        const showTable = () => {
            window.LaravelDataTables["tasks-assignment-table"].draw();
        };

        $('#filter-status, #filter-priority, #search-text-field').on('change keyup', function () {
            if ($('#filter-status').val() != "all" || $('#filter-priority').val() != "all" || $('#search-text-field').val() != "") {
