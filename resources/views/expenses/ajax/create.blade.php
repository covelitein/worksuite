                ...
                <div class="row p-20">
                    ...
                    <div class="col-md-6 col-lg-3">
                        @if(isset($projectName))
                            ...
                            <x-forms.text :fieldLabel="__('modules.invoices.currency')" fieldName="project-currency"
                                fieldId="project-currency" :fieldValue="$project->currency->currency_name"
                                fieldReadOnly="true" />
                        @else
                            ...
                            <x-forms.select :fieldLabel="__('modules.invoices.currency')" fieldName="currency"
                                ...
                                @foreach ($currencies as $currency)
                                    <option @selected ($currency->id == company()->currency_id) value="{{ $currency->id }}"
                                        data-currency-name="{{$currency->currency_code}}">
                    ...
                    <div class="col-md-6 col-lg-3">
                        <x-forms.number fieldId="exchange_rate"
                            :fieldLabel="__('modules.currencySettings.exchangeRate')" fieldName="exchange_rate"
                            fieldRequired="true" :fieldValue="(isset($projectName) ? $project->currency->exchange_rate : $companyCurrency->exchange_rate)" fieldReadOnly="true" :fieldHelp="' '" />
                    </div>
                    ...
                    <div class="col-md-6 col-lg-3">
                        <x-forms.select search="true" multiple="true" fieldId="department_id" fieldName="department_id"
                            :fieldLabel="__('app.department')" search="true">
                            <option value="" disabled>--</option>
                            @foreach ($departments as $department)
                                <option data-currency-id="{{ $department->id }}" value="{{ $department->id }}">
                    ...
                    <div class="col-md-6 col-lg-3">
                        <x-forms.select search="true" multiple="true" fieldId="project_id" fieldName="project_id" :fieldLabel="__('app.client')"
                            search="true">
                            <option value="" disabled>--</option>
                            @foreach ($clients as $client)
                                <option data-currency-id="{{ $client->id }}" value="{{ $client->id }}">
                                    {{ $client->name }}
                                </option>
                            @endforeach
                        </x-forms.select>
